(() => {
  ...
  const dateObj = m => { const d = new Date(dateValue(m)); return Number.isNaN(d.getTime()) ? null : d; };
  ...
  async function load() {
    try {
      ...
      const matches = [...rows(data.matches), ...rows(data.results), ...rows(data.result_summary)];
      const now = new Date();
      ...
      host.innerHTML = teams.map(team => {
        ...
        const future = relevant.filter(m => { const d=dateObj(m); return d && d >= now; }).sort((a,b)=>dateObj(a)-dateObj(b));
        const past = completed.filter(m => { const d=dateObj(m); return !d || d < now; }).sort((a,b)=>(dateObj(b)?.getTime()||0)-(dateObj(a)?.getTime()||0));
        const latest = past[0];
        const next = future[0];
        ...
        return `<a class="team-live-card" href="${team.href}">
          ...
          <div class="team-live-metrics"><div><span>Matches listed</span><strong>${played}</strong></div><div><span>${next ? 'Next' : 'Latest'}</span><strong>${esc(next ? formatDate(next) : latest ? formatDate(latest) : '—')}</strong></div></div>
