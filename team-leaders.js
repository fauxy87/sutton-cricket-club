(() => {
  ...
  function matchInvolvesTeam(detail) {
    ...
    return innings(detail).some(inn => battingTeamId(inn) === teamId);
  }
  ...
  async function load() {
    try {
        ...
        innings(detail).forEach(inn => {
          ...
          if (battingId === teamId) {
            (inn.bat || []).forEach(row => {
              ...
              const p = batting.get(id) || {
                id,
                name: row.batsman_name || row.name || 'Player',
                runs: 0,
                innings: 0,
                notOuts: 0,
                balls: 0,
              ...
              p.innings += 1;
              p.balls += balls;
              if (notOut) p.notOuts += 1;

              if (runs > p.high || (runs === p.high && notOut && !p.highText.endsWith('*'))) {
                p.high = runs;
            ...
            (inn.bowl || []).forEach(row => {
              ...
              const p = bowling.get(id) || {
                id,
                name: row.bowler_name || row.name || 'Player',
                wickets: 0,
                runs: 0,
                balls: 0,
                bestWickets: -1,
              ...
              if (wickets > p.bestWickets || (wickets === p.bestWickets && runs < p.bestRuns)) {
                p.bestWickets = wickets;
                p.bestRuns = runs;
                p.best = `${wickets}-${runs}`;
              }
      ...
      const batters = [...batting.values()].map(p => {
        const dismissals = Math.max(0, p.innings - p.notOuts);
        return {
          ...p,
          average: dismissals ? (p.runs / dismissals).toFixed(2) : '—',
          strikeRate: p.balls ? ((p.runs / p.balls) * 100).toFixed(1) : '—'
        };
      });

      const bowlers = [...bowling.values()].map(p => ({
        ...p,
        overs: ballsToOvers(p.balls),
        average: p.wickets ? (p.runs / p.wickets).toFixed(2) : '—',
        economy: p.balls ? (p.runs / (p.balls / 6)).toFixed(2) : '—'
      }));
      ...
      const card = (label, player, value, detail) => `<article class="season-stat player-leader-card">
        <span>${esc(label)}</span>
        <strong>${esc(value)}</strong>
        <p>${player ? playerLink(player.id, player.name) : '—'}</p>
