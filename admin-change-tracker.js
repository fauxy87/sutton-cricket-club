(()=>{
 ...
 function show(){if(dirty)return;dirty=true;bar.hidden=false;if(text)text.textContent='You have edited website content. Use the relevant Copy or Download button before publishing.';}
 ...
 document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;const id=b.id||'';if(id.includes('download')||id.includes('copy'))setTimeout(prepared,0);},true);
